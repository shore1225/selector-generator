...
export type SelectorGeneratorOptions = {
  root: Element;
...
let config: SelectorGeneratorOptions;

let rootDocument: Document | Element;
export function selectorGenerator(
  input: Element,
  options?: Partial<SelectorGeneratorOptions>
) {
  ...
  const defaults: SelectorGeneratorOptions = {
    root: document.body,
    idName: () => true,
    className: () => true,
    tagName: () => true,
...
function findRootDocument(
  ...
  defaults: SelectorGeneratorOptions
) {
...
/**
 * 自底向上找，并不断拼接 selector (优先级: id > attr > class > tagName)
 * 直到笛卡尔积中，出现能找到唯一目标元素的路径
 * 根据惩罚分数排序(id: 0, attr: 0.5, class: 1, tagName: 2, *: 3, nthChild: 额外 + 1)
 * 找到分数最低的路径并返回
 */
...
function attr(input: Element): Node[] {
  const attrs = Array.from(input.attributes).filter(a =>
    config.attr(a.name, a.value)
  );

  return attrs.map(
    (a): Node => ({
      name: `[${cssesc(a.name, { isIdentifier: true })}="${cssesc(a.value)}"]`,
      penalty: 0.5,
    })
  );
}
...
/**
 * 压缩路径，找出最短路径
 * 不断去除路径中的每一项，并确保还能找到唯一目标元素
 * 成功去除一项后，继续递归压缩，知道找出最短路径
 */
function* optimize(
  path: Path,
  input: Element,
...
/**
 * 转译特殊字符，详细见：https://www.npmjs.com/package/cssesc
 */
